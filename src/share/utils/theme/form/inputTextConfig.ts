...
function objectStyleToCssText(value: Record<string, string | number>): string | undefined {
  const declarations = Object.entries(value).map(([property, styleValue]) => `${toCssPropertyName(property)}: ${styleValue}`);
...
/**
 * Splits a pass-through attribute object into class and non-class attributes.
 *
